    ...
    <div className="max-w-6xl mx-auto space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Your podcast library and subscriptions ({mockPodcasts.length} episodes).
          </p>
        </div>
        <Button>
      ...
      <div className="space-y-4">
        {mockPodcasts.map((podcast) => (
          <Card key={podcast.id} className="group overflow-hidden transition-all duration-200 hover:shadow-md">
              ...
              <div className="flex">
                ...
                <div className="flex-1 p-4 flex flex-col justify-between min-h-[96px]">
                  <div className="flex-1">
                    <h3 className="font-semibold text-sm leading-tight mb-2 line-clamp-2">{podcast.title}</h3>
                    <p className="text-xs text-muted-foreground line-clamp-2 leading-relaxed">{podcast.description}</p>
                  </div>
                  <div className="flex items-center justify-between mt-3 pt-2">
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="truncate max-w-20">{podcast.host}</span>
                      </div>
                    </div>
                    <Button size="sm" variant="outline" className="h-7 px-3 text-xs bg-transparent">
                      {podcast.isPlayed ? "Played" : "Play"}
                    </Button>
                  </div>
                </div>
              </div>
